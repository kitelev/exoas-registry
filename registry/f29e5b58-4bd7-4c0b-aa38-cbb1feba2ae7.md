...
exo__AssetSpace descriptor for `kitelev/exoas-test` (EKA registry, D18 dependsOn). Holds the homoiconic plugin-testing TBox (`test__*` classes + properties). dependsOn `exoas-exo` (foundational metaclasses). Mount path `assetspaces/kitelev/exoas-test/`.
