...
exo__Asset_updatedAt: 2026-09-25T15:50:11
exo__Instance_class:
...
exo__AssetSpace_dependsOn:
  ...
  - "[[3c2fb27e-12a2-4793-a5dd-7a72c6e41def|exoas-lit]]"
  - "[[b9d37e39-5d58-4f09-9274-27fb138970f2|exoas-orgunit]]"
...
exo__AssetSpace_providesNamespace:
  ...
  - "[[9a38b39e-dbcc-4033-8c4e-74e15e7adcb9|agentskill]]"
---
...
> ⛤ **2026-09-12 (F4.2/M1, RFC `7353f80c`):** namespace **`exotemplate`** снят из этого
> каталога — он переехал в `kitelev/exoas-exo` (коммиты `exoas-exo@7203c72` + `exoas-public@0101394`).
> Основание: по `exo__Ontology_layer` это был единственный `Foundation` среди анкеров `exoas-public`.
> Переезд снял ребро `exoas-exocmd → exoas-public`. Классы `exotemplate__Template` и их 22 инстанса
> не менялись — изменился только дом определения.

> ⛤ **2026-09-12 (F4.3/M2 вариант C, RFC `7353f80c`):** namespace **`ztlk`** снят из этого
> каталога — он собран из ЧЕТЫРЁХ мест в новый публичный `kitelev/exoas-ztlk` (11 носителей).
> Переезд снял ребро `exoas-lit → exoas-public` и распустил `ЦИКЛ(2) exoas-lit ⇄ exoas-public`.
> Публичными стали только ИМЕНА классов; ABox (1118 инстансов `[три каноничных vault]`) не двигался.
...
## ztlk — zettelkasten notes — ПЕРЕЕХАЛ

⛔ **Namespace `ztlk` больше НЕ живёт в `exoas-public`.** 2026-09-12 (F4.3 / RFC `7353f80c` §M2,
вариант C) он собран целиком в отдельный публичный AssetSpace **`kitelev/exoas-ztlk`** — вместе
с вариантами `ztlk__PermanentNote` / `ztlk__FleetingNote` / `ztlk__FleetingNoteType`, которые
раньше лежали в `shared-identities`, и со свойствами из личного пространства.

⇒ Нужен Zettelkasten — монтируйте **`kitelev/exoas-ztlk`**, а не этот AssetSpace.
...
> ⛤ **2026-09-12 (F4.4 / M3, узел `1a1b20e3`): в `dependsOn` ДОБАВЛЕН `kitelev/exoas-lit`.**
> Основание — ФАКТ яруса определений, а не упаковочное удобство: `exoas-public → exoas-lit` = **2** трипла `exo__Property_range` — `ems__BookReadingSession_book → lit__Book` и `ems__SatsangSession_video → lit__WebPage`. Замер `[три каноничных vault]`, `--no-cache`, дедуп по триплу.
> ⛔ До этой правки ребро было фактическим и НЕобъявленным ⇒ профиль, включающий `exoas-public` без `exoas-lit`, оставлял `range` двух свойств нерезолвимым, а per-repo SHACL CI gate — тривиально проходящим (`RegistryDependencyResolver`: «trivially passes — a false-negative gate»).
